...
public class Scorer2013 {
	...
	Map<String,String> key_run = new HashMap<String,String>();
	...
	public  double[] run (String[] args) throws IOException {
		...
		while ((line = responseReader.readLine()) != null) {
			...
			if (!doc_id.equals("NIL")) {
				...
				confidence=Double.parseDouble(fields[8].trim());
			...
			if(doc_id.equals("NIL")){
				key = entity+"~"+slot+"~"+"NIL";
				output_string=entity+ "\t" + slot + "\t" + runid + "\t" + "NIL" + "\t" + "DUMMY" + "\t" + "DUMMY" + "\t" + "DUMMY" + "\t" + "DUMMY";
			}
			else{
				String ans = new String(fields[4].trim());
				key = entity+"~"+slot+"~"+ans;
				output_string=entity+ "\t" + slot + "\t" + runid + "\t" + doc_id + "\t" + ans  + "\t" + predoff + "\t" + entityoff + "\t" + filleroff;
			}
			String off = "";
			if(!fields[5].trim().contains(",")){
				off = fields[5].trim();
			}
			else{
				String[] tmp = fields[5].trim().split(",");
				off = tmp[0];
			}
			//key_run.put(key,runid);
			offset_rel.put(key,off);
			rel_prov.put(key, fields[3]);
		...
		double[] ans = new double[]{precision,recall,F};
