...
public class Scorer2014 {
	...
	Map<String,Integer> uiuc_feature = new HashMap<String,Integer>();
...
//			}
		...
		while ((line = responseReader.readLine()) != null) {
			...
			if(!fields[3].trim().contains(",")){
				rel_off = fields[3].trim().split(":")[0];
			}
			else{
				String[] tmp = fields[3].trim().split(",");
				rel_off = tmp[0].split(":")[0];
			}
			fil_rel_prov.put(key, rel_off);
			String off = "";
			if(!fields[3].trim().contains(",")){
				off = fields[3].trim().split(":")[1];
			}
			else{
				String[] tmp = fields[3].trim().split(",");
				off = tmp[0].split(":")[1];
			...
			fil_offset_rel.put(key,off);
			if(fields.length>5){
				rel_off = "";
				if(!fields[5].trim().contains(",")){
					rel_off = fields[5].trim().split(":")[0];
				}
				else{
